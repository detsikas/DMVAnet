...
train_dataset = get_dataset(training_dataset_files, training_dataset_dir, 1000, 8)
test_dataset = get_dataset(testing_dataset_files, testing_dataset_dir, 1000, 8)


input_shape = [256, 256, 3]
model_info = model_info_io.Info(input_shape, model_info_io.ModelType.UNET_PP)
model = models.unet_pp(input_shape, True)
adam_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001)
model.compile(optimizer=adam_optimizer, loss=losses.DiceLoss(),
              metrics=['binary_accuracy', 'mean_squared_error'])
print(model.summary())
#tf.keras.utils.plot_model(model, "multi_res_visual_attention_res_unet.png", show_shapes=True)
